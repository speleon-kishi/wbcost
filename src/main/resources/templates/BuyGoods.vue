                    ...
                    <table class="table">
                        ...
                        <tr v-for="(item,index) in items">
                            ...
                            <td>
                                ...
                                <input v-model="item.num" v-on:change="changeTotalPrice(index,$event)"
    ...
    module.exports = {
        ...
        methods: {
            ...
            changeTotalPrice(index, e) {
                ...
                var re = /^[0-9]+[0-9]*]*$/;
                if (!re.test(this.items[index].num)) {
                    alert("请输入自然数")
                    this.items[index].num = this.items[index].num.replace(/[^\d]/g, '') ? this.items[index].num.replace(/[^\d]/g, '') : 0;
                }
                if (this.items[index].num.length >= 2) {
                    this.items[index].num = this.items[index].num.replace(/^0+/g, "");
                }
                e.target.value = this.items[index].num;
                this.items[index].totalprice = this.items[index].num * this.items[index].price;
            },
            buyItem(item) {
